...
const LogOutput: React.FC<LogOutputProps> = ({ logs, onClear }) => {
  return (
    <div className="bg-white rounded-xl shadow-xl p-6 h-full">
      ...
      <div className="bg-gray-50 rounded-lg p-4 h-[calc(100vh-16rem)] overflow-y-auto font-mono text-sm">
        {logs.length === 0 ? (
          <p className="text-gray-500">Log output will appear here...</p>
        ) : (
          logs.map((log, index) => (
            <div key={index} className="text-gray-700 border-b border-gray-100 py-1 last:border-0">
              {log}
            </div>
          ))
        )}
